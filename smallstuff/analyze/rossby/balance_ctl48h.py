...
### Output file path
file_path = '/p/work1/lloveras/adj_4km/processed_june/bal_ctl_48h'
...
def bal(u_ctl, v_ctl, ph_ctl, nx, ny, dx, lam):
    
    du = horizontal_filter(np.nan_to_num(u_ctl), nx, ny, dx, lam)
    dv = horizontal_filter_v(np.nan_to_num(v_ctl), nx, ny, dx, lam)
    dph = horizontal_filter(np.nan_to_num(ph_ctl), nx, ny, dx, lam)
...
for i in range(len(ti)):
    ...
    if ti[i] <= 6:
        ...
        bal_pert[i] = bal(u_ctl,v_ctl,ph_ctl,nx,ny,dx,lam)
