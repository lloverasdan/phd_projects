...
@njit
def pi_grid(p_bot, pibot, pitop, npi_l, npi_h):
    ...
    for k in range(npi_l):
        ...
        p_pi_l[k] = p_bot*(pilvls_l[k]/CP)**(1/KAPPA)
    ...
    for k in range(npi_h):
        ...
        p_pi_h[k] = p_bot*(pilvls_h[k]/CP)**(1/KAPPA)
...
@njit
def pv_inv(pv, f, ny, p_bot, pibot, dy, npi, dpi, om):    

    fct = ((dy*dpi)**2.)*F0*p_bot/(G*KAPPA*(CP**(1/KAPPA)))
...
@njit
def pv_calc(f, ny, dy, npi, dpi, p_bot, pibot):

    pv_out = np.zeros((npi,ny))
    for k in range(1,npi-1):
        pilev = pibot - k*dpi
        for j in range(1,ny-1):
            pv_out[k,j] = (G*KAPPA*(CP**(1/KAPPA))/p_bot)*pilev**(1 - (1/KAPPA)) \
...
def solve_PV_inversion(ly, ny, dy, dytr, pim, dpitr, pvt, pvs, dpipv, npi, \
                       p_bot, pibot, pitop, dpi, dyth, thtop, dth, f, om, nit, \
    ...
    for i in range(nit):
        f = pv_inv(pv, f, ny, p_bot, pibot, dy, npi, dpi, om)
        f[0,:] = phbotline
    ...
    pv_out = pv_calc(f, ny, dy, npi, dpi, p_bot, pibot)
